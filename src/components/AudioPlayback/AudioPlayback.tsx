...
const AudioPlayback = ({ answer }: AudioPlaybackProps) => {
  ...
  const loadAudio = async (answer: string) => {
    ...
    try {
      ...
      const status = (await soundObj.getStatusAsync()) as AVPlaybackStatus;
  ...
  const getTimeRemaining = (duration: number, position: number) => {
    if (position > 0) {
  ...
  const getTimeElapsed = (position: number) => {
    if (position > 0) {
